...
#
# $Id: solvate.tcl,v 1.65 2019/08/27 22:48:31 johns Exp $
#
...
proc solvate {args} {
	...
	set invalidflags {-ks -spsf -spdb -stop -ws}
	...
	set modeltype [ lindex $args [expr $modelloc +1]]
	if { $modeltype=="" || [string range $modeltype 0 0] == "-" } {
	    set model tip3p
	    set args [lreplace $args $modelloc $modelloc ]
	} else {
	    set model [lindex $args [expr $modelloc+1]]
	    set args [lreplace $args $modelloc [expr $modelloc +1] ]

	}
	set availModel {tip3p tip4pew fb3 fb4 spce spceb opc}
	if {[lsearch $availModel $model]==-1} {
	    error "Solvent model $model is unavailable, use one of $availModel"
	}

	set wtpsf $env(SOLVATEDIR)/$model/${model}.psf
	set wtpdb $env(SOLVATEDIR)/$model/${model}.pdb
	set wttop $env(SOLVATEDIR)/$model/${model}_ions.str
	set ks {"name O"}
	
	vmdcon -info "Solvating using AMBER solvent model: [string toupper $model]"
	vmdcon -info "Running solvate command:"
	vmdcon -info "solvate $args -spsf $wtpsf -spdb $wtpdb -stop $wttop -ks $ks"
	vmdcon -info
	return [eval normalsolvate $args -spsf $wtpsf -spdb $wtpdb -stop $wttop -ks $ks ]
    }
}

proc normalsolvate {args} {
  ...
  if { [info exists psffile] } {
    if { $jsinput } {
      ...
      if {$rotate == 0} {
        ...
        readpsf $psffile pdb $pdbfile-rotated-tmp.pdb
