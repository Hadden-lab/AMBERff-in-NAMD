...
proc ::autoionize::autoionize_core {args} {
  ...
  if { [info exists cmdline(-nna)] && [info exists cmdline(-ncl)] } {
    ...
    foreach ion $IonNameDefaults charge {1 -1} num [list $nNa $nCl] {
  ...
  if {$mode == "sc" || $mode == "is"} {
    ...
    set sel [atomselect top "$watersel and noh"]
    set nWater [$sel num]
    $sel delete
  ...
  if {$done == 1} {
    ...
    while {1} {
      set ionList {}
      set sel [atomselect top "noh and $watersel and not (within $from of not $watersel)"]
      ...
      while {[llength $ionList] < $nIons} {
        ...
        if {[llength $ionList]} {
          ...
          set tempsel [atomselect top "$watersel and not $watersel"]
        }
        if {![$tempsel num]} {
